...
class StickyItemDecoration(private val mStickyHeadContainer: StickyHeadContainer, private val mStickyHeadType: Int) : ItemDecoration() {
    ...
    override fun onDraw(c: Canvas, parent: RecyclerView, state: RecyclerView.State) {
        ...
        if (mEnableStickyHead && mFirstVisiblePosition >= mStickyHeadPosition && mStickyHeadPosition != -1) {
            val belowView = parent.findChildViewUnder((c.width / 2).toFloat(), mStickyHeadContainer.childHeight + 0.01f)
            mStickyHeadContainer.onDataChange(mStickyHeadPosition)
            val offset = 0
            if (belowView != null && isStickyHead(parent, belowView) && belowView.top > 0) {
    ...
    private fun findFirstVisiblePosition(layoutManager: RecyclerView.LayoutManager?): Int {
        ...
        when (layoutManager) {
            is GridLayoutManager -> firstVisiblePosition = layoutManager.findFirstVisibleItemPosition()
            is LinearLayoutManager -> firstVisiblePosition = layoutManager.findFirstVisibleItemPosition()
            is StaggeredGridLayoutManager -> {
                val mInto = IntArray(layoutManager.spanCount)
    ...
    private fun findFirstCompletelyVisiblePosition(layoutManager: RecyclerView.LayoutManager): Int {
        var firstVisiblePosition = 0
        when (layoutManager) {
            is GridLayoutManager -> firstVisiblePosition = layoutManager.findFirstCompletelyVisibleItemPosition()
            is LinearLayoutManager -> firstVisiblePosition = layoutManager.findFirstCompletelyVisibleItemPosition()
            is StaggeredGridLayoutManager -> {
                val mInto = IntArray(layoutManager.spanCount)
                layoutManager.findFirstCompletelyVisibleItemPositions(mInto)
                firstVisiblePosition = Int.MAX_VALUE
                for (pos in mInto) {
                    firstVisiblePosition = Math.min(pos, firstVisiblePosition)
                }
